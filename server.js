...
async function mlFetch(endpoint, extraHeaders = {}, _isRetry = false) {
  ...
  // Leemos como texto primero: algunas respuestas de error de ML vienen vacías
  // o no son JSON, y eso rompía res.json() con un mensaje críptico.
  ...
  if (raw) {
    try {
      ...
      throw new Error(
        `Mercado Libre devolvió una respuesta inválida (status ${res.status}) en ${endpoint}: ` +
        (raw.slice(0, 200) || '(cuerpo vacío)')
      );
    }
  }
  ...
  if (!res.ok) {
    const msg = data?.message || data?.error ||
      `Error ${res.status} en ${endpoint}` + (raw ? `: ${raw.slice(0, 200)}` : ' (sin cuerpo de respuesta)');
    throw new Error(msg);
  }
...
async function getProductAdsReport(period) {
  ...
  try {
    ...
    throw new Error(`No se pudo obtener el advertiser de Product Ads: ${e.message}`);
  }
